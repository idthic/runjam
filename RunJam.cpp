...
namespace runjam {
  ...
  void RunJam::setWeakDecay() { libjam::setMSTC(42,0); }  //=0: allow weak decays
  void RunJam::unsetWeakDecay() { libjam::setMSTC(42,1); }  //=1:no weak decays
  void RunJam::setMSTC(int i, int j) { libjam::setMSTC(i,j); }
...
void RunJam::initialize(runjam_context const& ctx) {
  ...
  //....Initialize JAM
  libjam::setMSTC(1, ctx.get_config("runjam_jamseed", seed)); // int seed = 1921;
  libjam::setMSTC(2, this->nevent); // number of event.
  ...
  libjam::setMSTC(162,0);           // Output collision histroy
  ...
  std::string fn2 = libjam::getFNAME(2);
  std::string fn3 = libjam::getFNAME(3);
  std::string fn4 = libjam::getFNAME(4);
  if (outdir.size() > 0) {
    ...
    libjam::setFNAME(2, fn2.c_str());
    libjam::setFNAME(3, fn3.c_str());
    libjam::setFNAME(4, fn4.c_str());
    libjam::setFNAME(8, dir.c_str());
  }

  //...Initialize jam.
  libjam::jamInit(this->nevent, bmin, bmax, dt, nstep, "user", "p ", "p ", "2gev");
...
void outputPhaseSpaceBinary(std::ofstream& ofs) {
...
void RunJam::initJam(ParticleSampleBase* psamp) {
  ...
  libjam::setNBARY(nbary);    // set total number of baryon.
...
void RunJam::generateEvent(ParticleSampleBase* psamp, std::string const& cascadeMode) {
  ...
  aveNumberPart1 = 0.0;
  aveNumberPart2 = 0.0;
  libjam::setMSTC(5, numberTestParticle);
  ...
  for (int iev = 1; iev <= nevent; iev++) {
    ...
    if (ofs_bin0.is_open())
      outputPhaseSpaceBinary(ofs_bin0);
    ...
    if (flagDecayOnly) {
      ...
      if (iev % nprint == 0) {
        std::cout
          ...
          << (libjam::getMSTD(41) + libjam::getMSTD(42)) / numberTestParticle << "." << std::endl;
      }
    }

    aveNumberPart2 += (double) libjam::getNV() / (nevent * numberTestParticle);

    if (ofs.is_open())
      printPhaseSpaceData(ofs); // output the distribution to phasespace0.dat
    if (ofs_bin.is_open())
      outputPhaseSpaceBinary(ofs_bin);
  }
